...
public class CreateCollectionCommand implements Callable<Integer> {
	...
	private static final String CHROMOSOME = "hm_chr";

	private static final String POSITION = "hm_pos";
	...
	public Integer call() throws Exception {
		...
		for (int i = 0; i < filenames.length; i++){
			...
			formats[i] =  new PGSCatalogFormat(filenames[i], false);
		...
		String[] variantColumns = new String[]{"chr_name", "chr_position","effect_allele","other_allele"};
	...
	public Variant readVariant(ITableReader reader, RiskScoreFormatImpl format) {
		...
		variant.setChromosome(reader.getString(CHROMOSOME));
		if (reader.getString(POSITION).isEmpty()){
			throw new RuntimeException("Not position found.");
		}
		variant.setPosition(reader.getInteger(POSITION));
	...
	public void addVariant(ITableWriter writer, Variant variant){
		writer.setString("chr_name", variant.getChromosome());
		writer.setInteger("chr_position", variant.getPosition());
		writer.setString("effect_allele", variant.getEffectAllele());
		writer.setString("other_allele", variant.getOtherAllele());
	}
